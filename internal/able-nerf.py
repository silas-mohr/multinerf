...
from internal.transformers import *
...
class ableNeRF(LightningModule):
    def __init__(self, cfg, dim=256, ff_ratio=2, dropout=0.0, L_bands=16):
        ...
        self.view_token = nn.Sequential(nn.Linear(self.in_channels_dir, dim))
        ...
        self.coarse = TransformerClassDecoderV2(
            layers=point_coarse_layers, dim=dim, ff_ratio=ff_ratio, dropout=dropout
        )
        self.fine = TransformerClassDecoderV2(
            layers=point_fine_layers, dim=dim, ff_ratio=ff_ratio, dropout=dropout
        )
        ...
        self.c_spec_colour = LETransformer(
            dim=dim, ff_ratio=ff_ratio, dropout=dropout, lp_attn_layer=lp_attn_layer
        )
        ...
        self.f_spec_colour = LETransformer(
            dim=dim, ff_ratio=ff_ratio, dropout=dropout, lp_attn_layer=lp_attn_layer
        )
    ...
    def forward(self, rays):
        ...
        rgb_coarse = torch.clip(
            linear_to_srgb(rgb_coarse_spec_rgb + rgb_coarse_diff, eps), 0.0, 1.0
        )

        coarse_weights = attn_weights[
            :, 0, 1:
        ]  # 1 because discard self attn token weight
        ...
        rgb_fine = torch.clip(
            linear_to_srgb(rgb_fine_spec_rgb + rgb_fine_diff, eps), 0.0, 1.0
        )
