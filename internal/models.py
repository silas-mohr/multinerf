...
from internal import transformers
...
class Model(nn.Module):
    ...
    bg_intensity_range: Tuple[float] = (1.0, 1.0)  # The range of background colors.
    ...
    def __call__(
        ...
        for i_level in range(self.num_levels):
            ...
            logits_resample = jnp.where(
                sdist[..., 1:] > sdist[..., :-1],
                jnp.log(weights + self.resample_padding),
                -jnp.inf,
            )
...
class MLP(nn.Module):
    ...
    use_transformers: bool = False  # Use transformer module instead of directional MLP
    ...
    def __call__(
        ...
        if self.disable_density_normals:
            ...
            (
                raw_density_flat,
                x_flat,
            ), raw_grad_density_flat = predict_density_and_grad_fn(
        ...
        if self.disable_rgb:
            ...
            rgb = self.rgb_activation(
                self.rgb_premultiplier * dense_layer(self.num_rgb_channels)(x)
                + self.rgb_bias
            )
...
def render_image(
    render_fn: Callable[
        ...
        Tuple[List[Mapping[Text, jnp.ndarray]], List[Tuple[jnp.ndarray, ...]]],
    ],
    rays: utils.Rays,
    rng: jnp.array,
